...
namespace v1 {
    export function migrate(
        ...
        const localExerciseData = memento.get<LocalExerciseDataPartial[]>(
            data.v0.EXERCISE_DATA_KEY,
        );
        const courseExercises =
            localExerciseData && new Map(localExerciseData.map((x) => [x.id, x]));
        ...
        return localCourseData.map((course) => {
            const exercises = course.exercises.map((x) => {
                ...
                return {
                    ...x,
                    availablePoints:
                        x.availablePoints ?? LOCAL_EXERCISE_AVAILABLE_POINTS_PLACEHOLDER,
...
export default function migrateUserData(
    ...
    const dataV1 = dataV0
        ? { courses: v1.migrate(dataV0.courses, memento) }
        : validateData(
    ...
    const dataV2 = dataV1
        ? { ...dataV1, courses: v1.resolveMissingFields(dataV1?.courses) }
        : undefined;
