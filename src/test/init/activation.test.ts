...
// jest-mock-vscode ships neither the `env` nor the `extensions` namespace, and
// activation reads the editor name and the extension version from them.
...
vi.mock("../../api/langs", () => ({
  default: class {
    public isAuthenticated = async (): Promise<unknown> => Ok(false)
    public isMoocAuthenticated = async (): Promise<unknown> => Ok(false)
