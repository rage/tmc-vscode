...
suite("User data migration", function () {
  ...
  suite("between versions", function () {
    ...
    test("should succeed with version 0.6.0 data", async function () {
      ...
      const migratedCourse = migrateUserData(memento).data?.courses[0]
      expect(migratedCourse?.exercises.find((x) => x.id === 1)?.name).to.be.equal("hello_world")
      expect(migratedCourse?.exercises.find((x) => x.id === 2)?.name).to.be.equal("other_world")
    })
    ...
    test("should succeed with version 2.0.0 data", async function () {
      ...
      courses?.forEach((course) => {
        course.exercises.forEach((x) => {
          expect(x.availablePoints).to.be.equal(LOCAL_EXERCISE_AVAILABLE_POINTS_PLACEHOLDER)
          if (x.passed) {
            expect(x.awardedPoints).to.be.equal(LOCAL_EXERCISE_AWARDED_POINTS_PLACEHOLDER)
          } else {
            expect(x.awardedPoints).to.be.equal(LOCAL_EXERCISE_UNAWARDED_POINTS_PLACEHOLDER)
          }
        })
      })
    })
