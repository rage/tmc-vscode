...
{#if $errorData}
  <ul>
    {#if $errorData.tmcError}
      <li>
        ...
        <div>
          ...
          antivirus program. This may also be caused by an antivirus program. You can try to resolve
          this issue by adding an exception to the directory at '{$errorData.cliFolder}'.
        </div>
      </li>
    {/if}
    ...
    {#if $errorData.workspaceManagerError}
      <li>
        Failed to initialize workspace manager: {$errorData.workspaceManagerError.error}. Stack
        trace: {$errorData.workspaceManagerError.stack}
      </li>
    {/if}
...
<div>
  ...
  information about what went wrong. You can increase the log level the Settings to `verbose` to see
  more log messages.
</div>
<div>
