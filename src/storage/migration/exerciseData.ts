...
namespace v0 {
    export function exerciseIsClosed(
...
namespace v1 {
    export async function migrate(
        ...
        for (const exercise of exerciseData) {
            ...
            if (v0.exerciseIsClosed(status, isOpen)) {
            ...
            const pathResult = v0.resolveExercisePath(
                id,
                name,
                course,
                organization,
        ...
        for (const key of Object.keys(closedExercises)) {
            ...
            if (closeExercisesResult.err) {
                Logger.error(
                    "Failed to migrate status of closed exercises.",
                    closeExercisesResult.val,
                );
            }
        }
    }
...
export default async function migrateExerciseData(
    ...
    if (dataV0) {
        await v1.migrate(dataV0, memento, dialog, tmc);
