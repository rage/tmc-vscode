...
namespace v1 {
    function logLevelV0toV1(logLevel: data.v0.LogLevel): data.v1.LogLevel {
        switch (logLevel) {
...
namespace vscodeApi {
    ...
    export async function migrate(
        ...
        if (!version) {
            version = memento.get<SessionStatePartial>(data.v1.SESSION_STATE_KEY)?.extensionVersion;
        }
        ...
        if (!compareVersions || compareVersions < 0) {
            ...
            await settings.update(
                ...
                storageSettings.insiderVersion,
                true,
            );
...
export default async function migrateExtensionSettings(
    ...
    if (dataV1) {
        await vscodeApi.migrate(memento, dataV1, settings);
    }
